...
use App\Http\Controllers\ConfigCampaniaController;
...
|
*/
// ABRIR LA CONFIGURACION DE CAMPAÑAS

Route::get('/campanias', [ConfigCampaniaController::class, 'index'])
    ->name('campanias.index');


// Ruta usando controlador tradicional
Route::get('/confirmar-cita/{token?}', [CitaController::class, 'confirmar'])->name('confirmar.cita');

//Calendario

Route::get('calendario', [App\Http\Controllers\CalendarController::class, 'index'])->name('calendario.index');
Route::get('calendario/data', [App\Http\Controllers\CalendarController::class, 'getAppointments'])->name('calendario.data');
// Appointments
 
// Listado de citas (GET)
...
Route::middleware(['web', 'auth', 'impersonate'])->group(function () {
    ...
    // Toggle attendance (ASISTE) via AJAX (pure JS)
    Route::post('calendario/toggle-asiste', [App\Http\Controllers\CalendarController::class, 'toggleAsiste'])->name('calendario.toggle-asiste');
