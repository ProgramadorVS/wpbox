...
class Template extends Node
{
    public function listenForReply($message, $data)
    {
       ...
       // $template = ModelsTemplate::find($templateID);
        $template = ModelsTemplate::withoutGlobalScopes()->find($templateID);
    ...
    public function process($message, $data)
    {
        ...
        $templateID=$settings['selectedTemplateId'];
        //$template = ModelsTemplate::find($templateID);
         Log::info('Buscando template', ['templateID' => $templateID, 'tipo' => gettype($templateID)]); 
       $template = ModelsTemplate::withoutGlobalScopes()->find((string)$templateID);
      Log::info('Resultado de where', ['template' => $template]);
       //$template = ModelsTemplate::withoutGlobalScopes()->find($templateID);
        //Make an api call to our api to send the template message
        $components = [];

        Log::info('xxTemplate, settings,message,data,flow_id', ['template' => $template,'settings' => $settings,'message' => $message,'data' => $data,'flow_id' => $this->flow_id]);
        ...
        try {
            ...
            if (!$response->successful()) {
                Log::error('Failed to send template message', ['error' => $response->body()]);
                        ...
                        Log::info('TEMPLATE: MENSAJE A EDITAR PARA EL FLOW_HAS_REPLIED', ['keyword' => $messageData]);
                        if ($messageId) {
