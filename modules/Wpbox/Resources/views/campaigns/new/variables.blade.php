...
@foreach ($variables as $key => $itemBox)
    ...
    @if ($key=="header"||$key=="body")
        @foreach ($itemBox as $item)
            ...
            @include('partials.select',['id'=>"parammatch[".$key."][".$item['id']."]",'name'=>__('Seleccione el Campo a tomar'),'data'=>$contactFields, 'required'=>true,'value'=>"-2",'additionalInfos'=>"Use contact filed, as a value for the variable, or use static value."])
        @endforeach
    @elseif ($key=="document")
        ...
        @foreach ($itemBox as $button)
            @foreach ($button as $keybtn => $item)
                @if ($item['type']=="URL")
                    ...
                    @include('partials.select',['id'=>"parammatch[".$key."][".$keybtn."][".$item['id']."]",'name'=>__('Seleccione el Campo a tomar'),'data'=>$contactFields, 'required'=>true,'value'=>"-2",'additionalInfos'=>"Use contact filed, as a value for the variable, or use static value."])
