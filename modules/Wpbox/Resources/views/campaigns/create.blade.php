...
<div class="header  pb-8 pt-5 pt-md-8">
    <div class="container-fluid">
        <div class="header-body">
            @if ($isBot)
                <h1 class="mb-3 mt--3">🤖 {{__('Crear Respuesta Automática (Plantilla)')}}</h1>
            @elseif ($isAPI)
                <h1 class="mb-3 mt--3">🔌 {{__('Create new API campaign')}}</h1>
            @elseif ($isSimple)
                <h1 class="mb-3 mt--3">🔌 {{__('Crear Mensaje Cita')}}</h1>
            @else
            ...
            <div class="card shadow">
                <div class="card-header bg-white border-0">
                    <div class="row align-items-center">
                        <div class="col-8">
                            @if ($isBot)
                                <h3 class="mb-0">{{__('Respuesta Automática ( Plantilla )')}}</h3>
                            @elseif ($isAPI)
                                <h1 class="mb-3 mt--3">{{__('API campaign')}}</h1>
                            @elseif ($isSimple)
                                <h1 class="mb-3 mt--3">{{__('Mensaje Cita')}}</h1>
                            @else
                ...
                <div class="card shadow mt-4">
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            <div class="col-8">
                                ...
                                @elseif ($isSimple)
                                    <h3 class="mb-0">{{__('Guardar Mensaje Cita')}}</h3>
                                @else
                    ...
                    <div class="card-body">
                        @if (!isset($_GET['contact_id']))
                            ...
                            @elseif ($isSimple)
                                <p>{{__('Este mensaje se mandará cuando se configure ( ya sea cita o envío desde campaña)')}}</p>
                            @else
                                @if($selectedContacts!="")
                                    @if ($selectedContacts==1)
                                        <p>{{__('Mandar a')}}:{{$selectedContacts}} {{__('contacto')}}</p>
                                    @else
                                        <p>{{__('Mandar a')}}:{{$selectedContacts}} {{__('contactos')}}</p>
                                    @endif
                                @endif 
                            @endif
                        ...
                        @elseif ($isSimple)
                            <button  class="btn btn-success mt-4" type="submit">{{ __('Guardar Campaña Cita')}}</button>
                        @else
