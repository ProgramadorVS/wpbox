...
class Message extends Model
{
    ...
    {
        return $this->belongsTo(Appointment::class, 'cita_id', 'id','citas');
    }
    ...
    // Método estático para obtener el último mensaje por contact_id
    public static function getLastMessageByContactId($contactId)
    {
        return self::where('contact_id', $contactId)->latest()->first();
    }

    public static function getPenultimateMessageByContactId($contactId)
    {
        $messageCount = self::where('contact_id', $contactId)->count();

        if ($messageCount < 2) {
            // Retornar null si no hay suficientes registros
            return null;
        }
