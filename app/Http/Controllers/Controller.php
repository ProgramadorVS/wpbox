...
class Controller extends BaseController
{
    ...
    {
        if (! auth()->user()->hasRole('owner') && ! auth()->user()->hasRole('staff') && ! auth()->user()->hasRole('client')) {
            return null;
        }
    ...
    {
        if (! auth()->user()->hasRole('owner')) {
            abort(403, 'xUnauthorized action.');
        }
    }
    ...
    public function ownerAndStaffOnly()
    {
     ...
     if (!auth()->check()) {
        ...
        if (! auth()->user()->hasRole(['owner', 'staff', 'client'])) {
            abort(403, 'yUnauthorized action.');
        }
    }


   public function ownerAndClientOnly()
    {
        if (! auth()->user()->hasRole(['owner', 'client'])) {
    ...
    public function adminOnly()
    {
        if (! auth()->user()->hasRole('admin')) {
            abort(403, 'fUnauthorized action.');
        }
    }
